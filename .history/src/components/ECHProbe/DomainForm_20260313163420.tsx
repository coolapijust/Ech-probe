...
export function DomainForm({ onAnalyze, isLoading }: DomainFormProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    console.log('[DomainForm] handleSubmit called, domain:', domain, 'selectedPresets:', selectedPresets);
    if (domain) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-8 bg-card p-8 rounded-xl border shadow-xl">
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            {PRESET_RESOLVERS.map((res) => (
              <div key={res} className="flex items-center space-x-3 p-3 rounded-lg border bg-background/30 hover:bg-background/50 transition-colors cursor-pointer" onClick={(e) => { e.stopPropagation(); togglePreset(res); }}>
                <Checkbox checked={selectedPresets.includes(res)} onCheckedChange={() => togglePreset(res)} />
                <Label className="cursor-pointer">{res}</Label>
              </div>
            ))}
          </div>
        </div>
          ...
          <div className="flex flex-wrap gap-2">
            ...
            {customResolvers.map(url => (
              <Badge key={url} variant="secondary" className="py-1.5 px-3 flex items-center gap-2 group">
                ...
                <X className="w-3 h-3 cursor-pointer hover:text-destructive transition-colors" onClick={() => removeCustom(url)} />
              </Badge>
            ))}
          </div>
        </div>
      ...
      <Button 
        type="submit" 
        className="w-full h-14 text-lg font-headline bg-primary hover:bg-primary/90 text-white shadow-lg transition-all group"
        disabled={isLoading || !domain}
        onClick={() => console.log('[Button] clicked, isLoading:', isLoading, 'domain:', domain)}
      >
        {isLoading ? (
          <div className="flex items-center gap-2">
