...
export default function Home() {
  ...
  const handleAnalyze = async (domain: string, resolvers: string[], custom: string[]) => {
    console.log('[page.tsx] handleAnalyze called with:', { domain, resolvers, custom });
    setIsAnalyzing(true);
    setDomain(domain);
    try {
      const data = await performECHAnalysis(domain, resolvers, custom);
      console.log('[page.tsx] performECHAnalysis returned:', data);
      setResults(data);
    } catch (err) {
      console.error('[page.tsx] performECHAnalysis error:', err);
    } finally {
