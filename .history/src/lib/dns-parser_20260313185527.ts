...
function parseECHConfig(echConfig: Buffer, targetName: string): ECHConfigDetail {
  try {
    ...
    if (listLen > echConfig.length - offset || listLen < 10) {
      return createECHDetail(echConfig, targetName, 0, 'unknown');
    }

    // 第一个 ECHConfig 的长度 (2 bytes)
    const configLen = echConfig.readUInt16BE(offset);
    offset += 2;

    if (configLen > echConfig.length - offset || configLen < 8) {
      return createECHDetail(echConfig, targetName, 0, 'unknown');
    }
